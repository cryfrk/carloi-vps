...
async function upsertListingCompliance(postId, payload) {
  ...
  if (existing) {
    ...
    await statement.run(
      ...
      toDbBoolean(payload.authorizationDeclarationAccepted),
      payload.authorizationStatus || 'not_required',
      payload.eidsStatus || 'not_started',
      toDbBoolean(payload.safePaymentInfoAccepted),
      payload.safePaymentInfoAcceptedAt || null,
      payload.listingComplianceStatus || 'draft',
      payload.riskScore || 0,
      payload.riskLevel || 'low',
      toDbBoolean(payload.billingRequired),
      payload.billingStatus || 'not_required',
      payload.paymentRecordId || null,
      toDbBoolean(payload.duplicatePlateFlag),
      toDbBoolean(payload.abnormalPriceFlag),
      toDbBoolean(payload.spamContentFlag),
